...
const { Poru, Player } = require('poru');

// Safety patch: ensure Poru and Player never throw TypeError when query is null/undefined
if (Poru && Poru.prototype) {
    Poru.prototype.startsWithMultiple = function (s, words) {
        if (!s || typeof s !== 'string') return false;
        return words.some(w => s.startsWith(w));
    };

    const origResolve = Poru.prototype.resolve;
    Poru.prototype.resolve = async function (options, node) {
        if (!options || !options.query || typeof options.query !== 'string' || !options.query.trim()) {
            throw new Error('Please provide a valid song name or URL.');
        }
        return origResolve.call(this, options, node);
    };
}

if (Player && Player.prototype) {
    Player.prototype.startsWithMultiple = function (s, words) {
        if (!s || typeof s !== 'string') return false;
        return words.some(w => s.startsWith(w));
    };
}
...
function printHeader() {
    ...
    console.log(`${colors.CYAN}│${colors.RESET}                    ${colors.BOLD}${colors.PURPLE}♪ SAURAXT MUSIC ♪${colors.RESET}                    ${colors.CYAN}│${colors.RESET}`);
...
function printSystemReady() {
    printSeparator();
    console.log(`\n   ${colors.BOLD}${colors.PURPLE}♪ SauraXT Music Operational ♪${colors.RESET}`);
    console.log(`   ${colors.DIM}${colors.WHITE}Developed with ${colors.PINK}♡${colors.WHITE} for SauraXT${colors.RESET}`);
    console.log(`   ${colors.DIM}${colors.GRAY}Ready to bring high-quality music to your servers${colors.RESET}\n`);
    printSeparator();
    console.log();
}
...
const nodes = [];
const hosts = (config.LAVALINK.HOSTS || '').split(',').map(s => s.trim()).filter(Boolean);
const ports = (config.LAVALINK.PORTS || '').split(',').map(s => s.trim()).filter(Boolean);
const passwords = (config.LAVALINK.PASSWORDS || '').split(',').map(s => s.trim()).filter(Boolean);
const secures = (config.LAVALINK.SECURES || '').split(',').map(s => s.trim()).filter(Boolean);

for (let i = 0; i < hosts.length; i++) {
    nodes.push({
        name: `Node-${i + 1}`,
        host: hosts[i],
        port: parseInt(ports[i] || '443', 10),
        password: passwords[i] || 'youshallnotpass',
        secure: secures[i] === 'true'
    });
}
...
client.once('clientReady', async () => {
    ...
    try {
        client.user.setPresence({
            activities: [{ name: '🎵 /xtplay | SauraXT Music', type: 2 }],
            status: 'online'
        });
    } catch {}
...
client.on('interactionCreate', async (interaction) => {
    if (interaction.isChatInputCommand()) {
        ...
        const command = client.commands.get(interaction.commandName) || client.commands.get(baseName);
        if (!command) {
...
client.on('messageCreate', async (message) => {
    ...
    const prefixes = ['xt.', 'xt ', 'xt', config.PREFIX || ',', ','];
    for (const p of prefixes) {
        if (message.content.toLowerCase().startsWith(p.toLowerCase())) {
            messageContent = message.content.slice(p.length).trim();
            break;
        }
    }
    ...
    for (let wordCount = Math.min(allWords.length, 3); wordCount > 0; wordCount--) {
        ...
        const baseCmd = potentialCommand.startsWith('xt') ? potentialCommand.slice(2) : potentialCommand;
        if (client.prefixCommands.has(potentialCommand)) {
            ...
            break;
        } else if (client.prefixCommands.has(baseCmd)) {
            command = client.prefixCommands.get(baseCmd);
            commandName = baseCmd;
            args = allWords.slice(wordCount);
            break;
        }
...
async function registerCommands() {
    ...
    for (const command of client.commands.values()) {
        ...
        if (!cmdData.name.startsWith('xt')) {
            commands.push({ ...cmdData, name: `xt${cmdData.name}` });
        }
    }
